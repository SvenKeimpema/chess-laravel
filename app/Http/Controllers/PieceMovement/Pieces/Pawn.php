...
class Pawn implements Piece
{
    ...
    private function generateAttackingMoves(int $sq, bool $side, int $blocks, int $enemies): int
    {
        ...
        if ($side) {
            if ($sq >= 9) {
                $move1 = 1 << ($sq - 9);
                if (($enemies & $move1) !== 0 && ($move1 & $this->hFile) === 0) {
                    $bb |= $move1;
                }
            }
            ...
            if ($sq >= 7) {
                $move2 = 1 << ($sq - 7);
                if (($enemies & $move2) !== 0 && ($move2 & $this->aFile) === 0) {
                    $bb |= $move2;
                }
            }
        } else {
            if ($sq <= 54) {
                $move1 = 1 << ($sq + 9);
                if (($enemies & $move1) !== 0 && ($move1 & $this->aFile) === 0) {
                    $bb |= $move1;
                }
            }
            ...
            if ($sq <= 56) {
                $move2 = 1 << ($sq + 7);
                if (($enemies & $move2) !== 0 && ($move2 & $this->hFile) === 0) {
                    $bb |= $move2;
                }
            }
        }
