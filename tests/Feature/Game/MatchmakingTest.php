...
test('players can join the same game', function () {
    ...
    $game_user1 = UserGames::where('user_id', $user1->id)->first();
    $game_user2 = UserGames::where('user_id', $user2->id)->first();

    PHPUnit::assertTrue($game_user1 != null);
    PHPUnit::assertTrue($game_user2 != null);

    PHPUnit::assertTrue($game_user1->game_id == $game_user2->game_id);
